...
# WavLM分类模型
class WavLMForAudioClassification(nn.Module):
    """基于WavLM的音频分类模型"""

    def __init__(self, model_checkpoint, num_classes, hidden_dim):
        super(WavLMForAudioClassification, self).__init__()
        ...
        # 分类头 - WavLM优化架构
        self.dropout1 = nn.Dropout(0.2)  # WavLM更稳定，可适当降低dropout
...
# 绘制训练历史
def plot_training_history(history, save_path='wavlm_training_history.png'):
    """绘制训练历史"""
...
def main():
    ...
    SAMPLING_RATE = 16000  # WavLM标准采样率
    BATCH_SIZE = 16  # 可调节：WavLM相比HuBERT稍微高效，可适当提高批次大小
    HIDDEN_DIM = 768  # WavLM-base隐藏层维度
    MAX_SEQ_LENGTH = MAX_DURATION * SAMPLING_RATE
    MAX_EPOCHS = 40  # 可调节：最大训练轮数
    MODEL_CHECKPOINT = "microsoft/wavlm-base"  # WavLM预训练模型
    ...
    # 构建模型
    print("\n=== 构建WavLM模型 ===")
    model = WavLMForAudioClassification(MODEL_CHECKPOINT, NUM_CLASSES, HIDDEN_DIM)
    ...
    # 配置优化器和损失函数 - WavLM优化参数
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    optimizer = optim.AdamW(model.parameters(), lr=3e-5, weight_decay=0.01)  # WavLM稍高学习率
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), 'best_wavlm_model.pth')
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load('best_wavlm_model.pth'))
    ...
    # 保存模型和配置
    torch.save(model.state_dict(), 'wavlm_single_label_final.pth')
    config = {
        "model_type": "WavLM Single Label Classification",
        "cls_label": cls_label,
    ...
    with open('wavlm_large_config.json', 'w', encoding='utf-8') as f:
    ...
    print(f"\n=== WavLM 单标签训练完成 ===")
    print(f"模型权重已保存到: wavlm_single_label_final.pth")
    print(f"最佳模型已保存到: best_wavlm_model.pth")
    print(f"配置已保存到: wavlm_large_config.json")

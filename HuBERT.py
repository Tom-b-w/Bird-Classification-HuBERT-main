...
class HubertForAudioClassification(nn.Module):
    ...
    def __init__(self, model_checkpoint, num_classes, hidden_dim):
        super(HubertForAudioClassification, self).__init__()
        self.hubert = HubertModel.from_pretrained(model_checkpoint)
...
# 绘制训练历史
def plot_training_history(history, save_path='hubert_training_history.png'):
    """绘制训练历史"""
...
def main():
    ...
    SAMPLING_RATE = 16000  # HuBERT标准采样率
    BATCH_SIZE = 12  # 可调节：HuBERT内存占用较大，可适当降低批次大小
    HIDDEN_DIM = 768  # HuBERT-base隐藏层维度
    MAX_SEQ_LENGTH = MAX_DURATION * SAMPLING_RATE
    MAX_EPOCHS = 50  # 可调节：最大训练轮数
    MODEL_CHECKPOINT = "facebook/hubert-base-ls960"  # HuBERT预训练模型
    ...
    # 构建模型
    print("\n=== 构建HuBERT模型 ===")
    model = HubertForAudioClassification(MODEL_CHECKPOINT, NUM_CLASSES, HIDDEN_DIM)
    ...
    # 配置优化器和损失函数 - HuBERT优化参数
    criterion = nn.CrossEntropyLoss(weight=class_weights)
    optimizer = optim.AdamW(model.parameters(), lr=2e-5, weight_decay=0.01)  # HuBERT适用的学习率
    ...
    best_val_acc = 0
    patience = 0
    max_patience = 30  # HuBERT收敛可能较慢，增加耐心值
    ...
    config = {
        "model_type": "HuBERT Single Label Classification",
        "cls_label": cls_label,
    ...
    print(f"\n=== HuBERT 单标签训练完成 ===")
    print(f"模型权重已保存到: hubert_single_label_final.pth")
    print(f"最佳模型已保存到: best_hubert_model.pth")
    print(f"配置已保存到: hubert_large_config.json")

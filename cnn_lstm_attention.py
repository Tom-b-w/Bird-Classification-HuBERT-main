...
def shuffle_dataset(df, seed=42):
    ...
    # 显示最终数据分布
    print("最终数据分布:")
    sample_counts = shuffled_df['class_name'].value_counts()
    for species, count in sample_counts.items():
        print(f"  {species}: {count} 个样本")
...
# 特征提取
def extract_features(df, dataset_name="数据集", max_time_steps=65, n_features=40):
    """提取Mel频谱特征"""
    print(f"\n=== 提取{dataset_name}特征 ===")
    ...
    for i in tqdm(range(len(df)), desc=f"处理{dataset_name}"):
        audio_path = df['path'].iloc[i]
        label = df['class_label'].iloc[i]
        spec = get_spec(audio_path)
        if spec is not None:
            # 调整特征尺寸
            spec = np.resize(spec, (max_time_steps, n_features))
    ...
    if features:
        ...
        print(f"{dataset_name}特征形状: {features_array.shape}")
        return features_array, labels_array
    else:
        print(f"错误: {dataset_name}没有成功提取到任何特征")
...
# 绘制训练历史
def plot_training_history(history, save_path='single_label_training_history.png'):
    """绘制训练历史"""
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 5))

    ax1.plot(history.history['accuracy'], label='训练准确率', linewidth=2, color='blue')
    ax1.plot(history.history['val_accuracy'], label='验证准确率', linewidth=2, color='red')
    ...
    ax2.plot(history.history['loss'], label='训练损失', linewidth=2, color='blue')
    ax2.plot(history.history['val_loss'], label='验证损失', linewidth=2, color='red')
...
def predict_single_audio(model, audio_path, cls_label, max_time_steps=65, n_features=40):
    ...
    try:
        ...
        if spec is not None:
            spec = np.resize(spec, (max_time_steps, n_features))
            spec = spec.reshape((1, max_time_steps, n_features))

            # 预测
            pred_probs = model.predict(spec, verbose=0)
            ...
            predicted_species = idx_to_species.get(predicted_class, "未知")
...
def main():
    ...
    def name_to_label(x):
        ...
        return cls_label.get(cls_name, -1)
    ...
    if df_train.empty or df_val.empty:
        print("错误: 数据平衡处理后数据集为空")
        return None

    print(f"平衡后 - 训练集: {len(df_train)}, 验证集: {len(df_val)}")

    # 提取特征
    X_train, y_train = extract_features(df_train, "训练集", MAX_TIME_STEPS, N_FEATURES)
    X_val, y_val = extract_features(df_val, "验证集", MAX_TIME_STEPS, N_FEATURES)
    ...
    print("✓ 最佳模型已保存到: best_label_model.keras")
    print("✓ 配置已保存到: single_label_bird_config.json")


    # 最终总结
    print(f"\n=== 单标签鸟类识别训练完成 ===")
    print(f"模型类型: 单标签多类分类")

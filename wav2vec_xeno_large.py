...
def main():
    ...
    # 加载最佳模型
    model.load_state_dict(torch.load('best_wav2vec2_model.pth'))
    ...
    # 保存模型和配置
    torch.save(model.state_dict(), 'wav2vec2_single_label_final.pth')
    config = {
    ...
    with open('wav2vec2_single_label_config.json', 'w', encoding='utf-8') as f:
    ...
    print(f"\n=== Wav2Vec2 单标签训练完成 ===")
    print(f"模型权重已保存到: wav2vec2_single_label_final.pth")
    print(f"最佳模型已保存到: best_wav2vec2_model.pth")
    print(f"配置已保存到: wav2vec2_single_label_config.json")
